...
#pragma omp task depend(out: A[i][j]) depend(in:A[i][j-1]) firstprivate(i,j)
	...
	#pragma omp task depend(out: A[i][j]) depend(in:A[i-1][j]) firstprivate(i,j)
	tache(i,j);
  
      }else{
#pragma omp task depend(out: A[i][j]) depend(in:A[i-1][j]) depend(in:A[i][j-1]) firstprivate(i,j)
      tache(i,j);
      }
