		  ...
		  calc_t radius, unsigned natoms, unsigned offset)
{
  ...
  if (pos[index] <= min[c] || pos[index] >= max[c]){
...
{
	for(int i = index + 1; i < natoms ; i++){
	coord_t mine = load3coord(pos+index,offset);
	coord_t current = load3coord(pos+i,offset);
		if(distance(mine,current) <= 2*radius){
...
// This kernel is executed with one thread per atom
// gx, gy, gz : vecteur qui pointe vers le ciel
__kernel
void gravity (__global calc_t * speed, calc_t gx, calc_t gy, calc_t gz,
	      unsigned natoms, unsigned offset)
{
    unsigned index = get_global_id (0);
    coord_t mine = load3coord(speed + index,offset);
    mine.x -= gx;
    mine.y -= gy;
    mine.z-= gz;
    store3coord(speed + index, mine , offset);
}
