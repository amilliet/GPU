...
static void seq_update_vbo (sotl_device_t *dev)
{
  ...
  for (unsigned n = 0; n < set->natoms; n++) {
    ...
    // Atom color depends on (z coordinate) last shock with others 
    {
      ...
      if (atom_state == 50)
	vbo_color[n*3 + 2] =  atom_color[0].R;
      else
...
static void seq_gravity (sotl_device_t *dev)
{
  ...
  const calc_t g = 0.005;
   ...
   for (unsigned n = 0; n < set->natoms; n++) {
     set->speed.dx[n] -= g*normalized_vert[0];
     set->speed.dy[n] -= g*normalized_vert[1];
     set->speed.dz[n] -= g*normalized_vert[2];
   }
}
